...
# ============================================================
# train.py — Multi-scale avec entropie + hard reset
# ============================================================
...
def init_prototypes_from_data(population, images, labels, device, n_samples=50):
    """Initialisation STRICTE avec filtrage Top 25% variance."""
    print(f"Initialisation STRICTE (intensité: {population.use_intensity})...")
...
def run_experiment(train_images, train_labels, val_images, val_labels,
                   ...
                   theta_init=0.5, K=1, device="cuda",
                   use_intensity=True,
                   entropy_weight=0.1,  # ✅ Nouveau
                   hard_reset_every=2,  # ✅ Nouveau
                   hard_reset_threshold=0.70,  # ✅ Nouveau
                   save_viz_every=5, viz_dir="figs/epoch_viz"):
    """
    Entraînement avec :
    - Gradient descent vectorisé GPU
    - Pénalité entropie (forcer spécialisation)
    - Hard reset prototypes ambigus tous les N epochs
    """
    
    # Images fixes pour visualisation
    viz_indices = [0, 10, 20, 30, 40, 100, 150, 200, 250, 300]
    viz_images = [val_images[i] for i in viz_indices]
    viz_labels = [val_labels[i] for i in viz_indices]
    ...
    for epoch in range(epochs):
        ...
        # ✅ HARD RESET tous les N epochs
        if epoch > 0 and epoch % hard_reset_every == 0:
            print(f"\n  [Epoch {epoch+1}] Hard reset prototypes ambigus...")
            pop.hard_reset_ambiguous(threshold=hard_reset_threshold)
        
        # ✅ Training avec pénalité entropie
        trainer.train_batch(train_images, train_labels, 
                           batch_size=2, lr=lr_epoch, 
        ...
        # Visualisations périodiques
        if (epoch + 1) % save_viz_every == 0:
            save_epoch_visualizations(
                pop=pop, trainer=trainer,
                val_images=viz_images, val_labels=viz_labels,
                epoch=epoch + 1, class_names=["Cancer", "Normal"],
                save_dir=viz_dir, n_examples=10
            )
