...
def save_epoch_visualizations(pop, trainer, val_images, val_labels, 
                              epoch, class_names, save_dir, n_examples=5):
    """
    Sauvegarde visualisations pour n_examples images à cet epoch.
    
    Args:
        pop: PopulationBMultiScale
        trainer: TrainerMultiScale
        val_images: Liste images validation
        val_labels: Liste labels validation
        epoch: Numéro epoch actuel
        class_names: ["Cancer", "Normal"]
        save_dir: Dossier de base (ex: "figs/epoch_viz")
        n_examples: Nombre d'images à visualiser
    """
    epoch_dir = os.path.join(save_dir, f"epoch_{epoch:03d}")
    os.makedirs(epoch_dir, exist_ok=True)
    
    # Prédire sur validation
    preds = trainer.predict_batch(val_images[:20], batch_size=4)  # Pred sur 20 premières
    
    # Sélectionner exemples variés
    selected = []
    
    # Chercher 1 correct Cancer, 1 correct Normal, 1 erreur chaque
    for i, (pred, true) in enumerate(zip(preds[:20], val_labels[:20])):
        if pred is None:
            continue
        ...
        # Diversifier
        if pred == 0 and true == 0 and not any(s[2] == 0 and s[3] == 0 for s in selected):
            selected.append((i, pred, true, "correct_cancer"))
        elif pred == 1 and true == 1 and not any(s[2] == 1 and s[3] == 1 for s in selected):
            selected.append((i, pred, true, "correct_normal"))
        elif pred == 1 and true == 0 and not any(s[2] == 0 and s[3] == 1 for s in selected):
            selected.append((i, pred, true, "error_cancer_to_normal"))
        elif pred == 0 and true == 1 and not any(s[2] == 1 and s[3] == 0 for s in selected):
            selected.append((i, pred, true, "error_normal_to_cancer"))
    
    # Compléter avec images random si pas assez
    for i, (pred, true) in enumerate(zip(preds[:20], val_labels[:20])):
        if len(selected) >= n_examples:
            break
        if pred is not None and i not in [s[0] for s in selected]:
            status = "correct" if pred == true else "error"
            selected.append((i, pred, true, status))
    
    # Générer visualisations
    print(f"\n  [Epoch {epoch}] Génération {len(selected)} visualisations...")
    
    for idx, pred, true, status in selected:
    ...
    print(f"  [Epoch {epoch}] ✅ {len(selected)} images sauvegardées dans {epoch_dir}/")